...
#ifndef EACH_VLAN_SET
#define EACH_VLAN_SET(_vlans, _vlan, _set)                                       \
	for ((_vlan) = VLAN_MIN; (_vlan) <= VLAN_MAX                               \
		     && (((_set) = CHECK_VLAN((_vlans), (_vlan))), 1);                \
	     (_vlan)++)                                                             \
		if (!(_set)) {} else
#endif
...
#ifndef CLEAR_VLAN
#define CLEAR_VLAN(_vlans, _vlan)                                                \
	((_vlans)[(_vlan) / 8] &= ~(1U << ((_vlan) % 8)))
#endif

#ifndef MERGE_VLANS
#define MERGE_VLANS(_dst, _idx, _src)                                            \
	do {                                                                       \
		for ((_idx) = 0; (_idx) < VLANS_ARRSIZE; (_idx)++)                  \
			(_dst)[(_idx)] |= (_src)[(_idx)];                              \
	} while (0)
#endif
...
#ifndef MAX_VLANS_SUBTLV_LEN
#define MAX_VLANS_SUBTLV_LEN (UINT8_MAX - sizeof(struct trill_enabledvlans_subtlv))
#endif

#ifndef REVERSE_BYTE
#define REVERSE_BYTE(_b)                                                     \
	((((_b) & 0x01U) << 7) | (((_b) & 0x02U) << 5) |                    \
	 (((_b) & 0x04U) << 3) | (((_b) & 0x08U) << 1) |                    \
	 (((_b) & 0x10U) >> 1) | (((_b) & 0x20U) >> 3) |                    \
	 (((_b) & 0x40U) >> 5) | (((_b) & 0x80U) >> 7))
#endif
...
#ifndef VLANTCI
#define VLANTCI(_tci) ((_tci) & 0x0FFF)
#endif

#ifndef PORT_CAPABILITY
#define PORT_CAPABILITY 242
#endif
...
#ifndef PCSTLV_VLANS_LEN
#define PCSTLV_VLANS_LEN 4
#endif

#ifndef PCSTLV_APPFORWARDERS
#define PCSTLV_APPFORWARDERS 2
#endif

#ifndef PCSTLV_ENABLEDVLANS
#define PCSTLV_ENABLEDVLANS 3
#endif

#ifndef PCSTLV_ENABLEDVLANS_MIN_LEN
...
int tlv_add_trill_vlans(struct isis_circuit *circuit);

static int
add_tlv(uint8_t tlv, size_t len, const uint8_t *value, struct stream *stream)
{
  if (len > UINT8_MAX)
    return ISIS_WARNING;
...
static int
add_subtlv(uint8_t subtlv, size_t len, const uint8_t *value,
...
trill_compute_vlanfwders(struct isis_circuit *circuit, int *refresh)
{
  ...
  uint32_t prevhash = circuit->vlans->vlanfwdershash;
  uint32_t newhash = 0;
...
send_trill_vlan_hellos(struct isis_circuit *circuit)
{
  ...
  struct list *adjdb;
  struct isis_adjacency *adj;
  uint8_t txvlans[VLANS_ARRSIZE];
  uint8_t fwdvlans[VLANS_ARRSIZE];
  unsigned int vlan_set;
  int vlan;
  ...
  if (circuit->u.bc.is_dr[TRILL_ISIS_LEVEL - 1])
    {
      ...
      if (refresh_lsp)
        lsp_regenerate_schedule(circuit->area, TRILL_ISIS_LEVEL, false);
  ...
  else 
    {
       ...
       */
      memset(txvlans, 0, VLANS_ARRSIZE);
...
static int
vertex_has_parent(struct isis_vertex *vertex, struct isis_vertex *parent)
{
  struct listnode *node;
  struct isis_vertex *p;

  if (vertex == NULL || parent == NULL || vertex->parents == NULL)
    return FALSE;

  for (ALL_LIST_ELEMENTS_RO(vertex->parents, node, p))
    if (memcmp(p->N.id, parent->N.id, ISIS_SYS_ID_LEN) == 0)
      return TRUE;
  ...
  struct isis_vertex *vertex, struct isis_vertex *rvertex, uint8_t *filtermap)
{
  ...
  if (isis_find_vertex(&rdtree->paths, &vertex->N, vertex->type) == NULL)
    return;
  ...
  for (ALL_QUEUE_ELEMENTS_RO (&rdtree->paths, node, cvertex))
    {
      if (!vertex_has_parent(cvertex, vertex))
        continue;
...
trill_create_vlanfilterlist(struct isis_area *area, nicknode_t *nicknode)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (adjlist, node, listdata))
    {
      adjnick = (uint16_t)(unsigned long)listdata;
      ...
      for (ALL_QUEUE_ELEMENTS_RO (&rdtree->paths, lnode, vertex))
        {
          if (vertex->type != VTYPE_NONPSEUDO_IS &&
	      vertex->type != VTYPE_NONPSEUDO_TE_IS)
	    continue;
	   ...
	   */
		...
		isis_vertex_queue_last(&rdtree->paths), adjvertex, vlanfiltermap);
        }
      else
        {
	  /*
...
static void
trill_parse_enabled_vlans_subtlv(struct isis_adjacency *adj, uint8_t *ptr,
	uint8_t len)
{
   uint8_t *end;
...
static void
remove_inhib(struct isis_circuit *circuit, uint16_t rxvlan)
{
  ...
  if (list_isempty (cvlans->inhibit_vlans) && cvlans->inhibit_all == 0 &&
    ...
    {
      event_cancel(&cvlans->inhibit_thread);
      cvlans->inhibit_thread = NULL;
    }
}
 ...
 */
static void
check_disabled_inhib (struct isis_circuit *circuit, uint8_t *prevvlans,
    uint8_t *newvlans)
{
  struct trill_inhibit_vlan *inhib;
  struct listnode *node, *nextnode;
...
static void
uninhibit_vlan (struct event *thread)
{
  ...
  if (node == NULL && cvlans->inhibit_all == 0)
	...
	mintime = alltime;
      event_add_timer(master, uninhibit_vlan, circuit, mintime,
		      &cvlans->inhibit_thread);
    }
  return;
}
...
{
  uint8_t nullvlans[VLANS_ARRSIZE];
  struct trill_circuit_vlans *cvlans = circuit->vlans;
  int interval;
  ...
  event_add_timer(master, uninhibit_vlan, circuit, interval,
		&cvlans->inhibit_thread);
}

static void
add_inhib(struct isis_circuit *circuit, uint16_t rxvlan)
{
  struct trill_circuit_vlans *cvlans = circuit->vlans;
  struct trill_inhibit_vlan *inhib;
  int interval;
...
trill_process_hello(struct isis_adjacency *adj, struct list *portcaps)
{
  ...
  if (adj_is_dr)
    {
      uint8_t *fwdvlans;
      uint8_t *enabledvlans;
      uint8_t prevfwdvlans[VLANS_ARRSIZE];
      uint8_t appvlans[VLANS_ARRSIZE];
      ...
      memset(appvlans, 0, sizeof (appvlans));
       ...
       "VLAN ID\n")
{
  ...
  SET_VLAN(circuit->vlans->enabled, atoi(argv[0]->arg));
  return CMD_SUCCESS;
}
       ...
       "VLAN ID\n")
{
  ...
  CLEAR_VLAN(circuit->vlans->enabled, atoi(argv[0]->arg));
  return CMD_SUCCESS;
}
       ...
       "PVID\n")
{
  ...
  circuit->vlans->pvid = atoi(argv[0]->arg);
  return CMD_SUCCESS;
}
