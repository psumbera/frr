...
#ifndef TRILL_FLAGS_V0
#define TRILL_FLAGS_V0 0x01
#endif

#ifndef TRILL_STR
#define TRILL_STR "TRILL information\n"
#endif

#ifndef TRILL_NICK_STR
#define TRILL_NICK_STR "TRILL nickname\n"
#endif

#ifndef CHECK_VLAN
#define CHECK_VLAN(_vlans, _vlan)                                                \
	(((_vlans)[(_vlan) / 8] & (1U << ((_vlan) % 8))) != 0)
#endif

#ifndef EACH_VLAN_R
#define EACH_VLAN_R(_vlans, _vlan, _set)                                         \
	for ((_vlan) = 0; (_vlan) <= VLAN_MAX                                       \
		     && (((_set) = CHECK_VLAN((_vlans), (_vlan))), 1);                \
	     (_vlan)++)
#endif
...
#ifndef SET_VLAN
#define SET_VLAN(_vlans, _vlan) ((_vlans)[(_vlan) / 8] |= (1U << ((_vlan) % 8)))
#endif

#ifndef RCSTLV_TRILL_NICKNAME
#define RCSTLV_TRILL_NICKNAME 6
#endif

#ifndef RCSTLV_TRILL_FLAGS
#define RCSTLV_TRILL_FLAGS 7
#endif

#ifndef RCSTLV_TRILL_VLANSROOTS
#define RCSTLV_TRILL_VLANSROOTS 10
#endif

struct trill_nickname_subtlv_compat {
	uint8_t tn_priority;
...
struct trill_vlan_bridge_roots_subtlv_compat {
	uint16_t vlan_start;
	uint16_t vlan_end;
} __attribute__((packed));

static int add_tlv(uint8_t tlv, size_t len, const uint8_t *value,
		   struct stream *stream)
{
	if (len > UINT8_MAX)
		return ISIS_WARNING;

	if (STREAM_WRITEABLE(stream) < (ssize_t)(2 + len))
		return ISIS_WARNING;

	stream_putc(stream, tlv);
	stream_putc(stream, len);
	if (len && value)
		stream_put(stream, value, len);

	return ISIS_OK;
}

static int add_subtlv(uint8_t subtlv, size_t len, const uint8_t *value,
		      size_t tlv_start, struct stream *stream)
{
	uint8_t tlv_len;
	size_t avail;
	...
	if (tlv_start + 1 >= (size_t)stream_get_endp(stream))
		return ISIS_WARNING;

	tlv_len = stream_getc_from(stream, tlv_start + 1);
	if ((size_t)tlv_len + 2 + len > UINT8_MAX)
		return ISIS_WARNING;

	avail = STREAM_WRITEABLE(stream);
	if (avail < (2 + len))
		return ISIS_WARNING;

	stream_putc(stream, subtlv);
	stream_putc(stream, len);
	if (len && value)
		stream_put(stream, value, len);

	stream_putc_at(stream, tlv_start + 1, tlv_len + 2 + len);
	return ISIS_OK;
}

extern struct zebra_privs_t isisd_privs;

static struct isis *trill_first_instance(void)
{
	if (!im || !im->isis || listcount(im->isis) == 0)
		return NULL;

	return listgetdata(listhead(im->isis));
}

static const char *sysid_print(const uint8_t *sysid)
{
	return print_sys_hostname(sysid);
}

static const char *snpa_print(const uint8_t *snpa)
{
	static char buf[32];

	snprintfrr(buf, sizeof(buf), "%pEA", snpa);
	return buf;
}
...
static int
trill_nickname_nickbitmap_op(uint16_t nick, int update, int val)
{
...
trill_nickname_alloc(void)
{
  unsigned int i, j, k;
  uint16_t nick;
  unsigned int nicknum;
  unsigned int freenickcnt = 0;
...
{
  return (memcmp(key1, key2, sizeof(uint16_t)));
}
 ...
 */
int
trill_area_nickname(struct isis_area *area, uint16_t nickname)
{
  int savednick;
  ...
  if (nickname == RBRIDGE_NICKNAME_NONE)
    {
      ...
      lsp_regenerate_schedule(area, TRILL_ISIS_LEVEL, false);
      return TRUE;
    }
  ...
  if (is_nickname_used(nickname))
    {
      ...
      memcpy(ni.sysid, area->isis->sysid, ISIS_SYS_ID_LEN);
...
static void
trill_nickname_priority_update(struct isis_area *area, uint8_t priority)
{
  if (priority)
    {
...
trill_dict_create_nodes (struct isis_area *area, nickinfo_t *nick)
{
  ...
  tnode->rdtree = isis_spftree_new(area, &area->lspdb[TRILL_ISIS_LEVEL - 1], area->isis->sysid, TRILL_ISIS_LEVEL, SPFTREE_IPV4, SPF_TYPE_FORWARD, 0, 0);
  /* clear copied nick */
...
trill_nick_destroy(struct isis_lsp *lsp)
{
  ...
  isis = trill_first_instance();
  if (!isis || !isis->trill_active || isis->area_list->count == 0)
    return;

  area = listgetdata(listhead(isis->area_list));
  lsp_id = lsp->hdr.lsp_id;
	  ...
	  (ni.nick.name == RBRIDGE_NICKNAME_NONE))
    {
      ...
      if (IS_DEBUG_EVENTS)
        zlog_debug("ISIS TRILL removed any nickname associated"
	      ...
	      sysid_print(lsp_id), ntohl (lsp->hdr.seqno),
...
{
  XFREE (MTYPE_ISIS_TRILL_VLANBRIDGE_ROOTS, data);
}
...
trill_parse_lsp (struct isis_lsp *lsp, nickinfo_t *recvd_nick)
{
  ...
  if (lsp == NULL || lsp->tlvs == NULL || lsp->tlvs->router_cap == NULL)
    return FALSE;

  /*
   * FRR TLV parsing no longer exposes raw Router Capability sub-TLV payloads.
   * Solaris TRILL-specific sub-TLV decoding needs follow-up adaptation.
   */
  return FALSE;
}
		...
		struct isis_lsp *lsp)
{
  ...
  if ((memcmp (lsp->hdr.lsp_id, area->isis->sysid, ISIS_SYS_ID_LEN) == 0)
       || (LSP_PSEUDO_ID(lsp->hdr.lsp_id) != 0))
    return;
...
u_char *
nick_to_sysid(struct isis_area *area, uint16_t nick)
{
  nicknode_t *tnode;
...
trill_create_nickfwdtable(struct isis_area *area)
{
  ...
  rdtree = area->spftree[SPFTREE_IPV4][TRILL_ISIS_LEVEL - 1];
  if (!rdtree)
    return;
  ...
  for (ALL_QUEUE_ELEMENTS_RO (&rdtree->paths, node, vertex))
    {
      if (firstnode)
        {
      ...
      if (listhead (vertex->Adj_N) &&
          (vadj = listgetdata (listhead (vertex->Adj_N))) &&
          (sadj = vadj->sadj) != NULL && sadj->adj)
        {
          fwdnode = XCALLOC (MTYPE_ISIS_TRILL_FWDTBL_NODE, sizeof(nickfwdtblnode_t));
          fwdnode->dest_nick = sysid_to_nick (area, vertex->N.id);
          memcpy(fwdnode->adj_snpa, sadj->adj->snpa, sizeof(fwdnode->adj_snpa));
          fwdnode->interface = sadj->adj->circuit->interface;
		 ...
		 struct isis_vertex *vertex)
{
  ...
  if (!nick)
    return;
  if (listnode_lookup (adjlist, (void *)(unsigned long)nick) != NULL)
    return;
  listnode_add (adjlist, (void *)(unsigned long)nick);
}
...
trill_create_nickadjlist(struct isis_area *area, nicknode_t *nicknode)
{
  ...
  /* Determine adjacencies from SPF next-hop adjacencies. */
  if (rbvertex)
    {
      adjlist = list_new();
      for (ALL_LIST_ELEMENTS_RO (rbvertex->Adj_N, anode, vadj))
        {
          if (!vadj || !(sadj = vadj->sadj))
            continue;
          if (memcmp (sadj->id, area->isis->sysid, ISIS_SYS_ID_LEN) == 0)
            continue;

          vertex = isis_find_vertex (&rdtree->paths, sadj->id,
                                     VTYPE_NONPSEUDO_IS);
          if (!vertex)
            vertex = isis_find_vertex (&rdtree->paths, sadj->id,
...
trill_fwdtbl_lookup (struct isis_area *area, uint16_t nick)
{
  struct listnode *node;
  nickfwdtblnode_t *fwdnode;
...
trill_adjtbl_print (struct vty *vty, struct isis_area *area, nicknode_t *nicknode)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (adjnodes, node, listdata))
    {
      nick = (uint16_t)(unsigned long)listdata;
...
trill_adjtbl_print_all (struct vty *vty, struct isis_area *area)
{
  ...
  for (dnode = dict_first(area->trill->nickdb);
       dnode != NULL;
       dnode = dict_next(area->trill->nickdb, dnode))
    {
      tnode = dnode_get(dnode);
...
static void
trill_publish_nick(struct isis_area *area, int fd, uint16_t nick,
    nickfwdtblnode_t *fwdnode)
{
  ...
  size = sizeof(trill_nickinfo_t) + (adjcount * sizeof (uint16_t)) +
...
void
trill_set_vlan_forwarder (struct isis_circuit *circuit, uint8_t *forwarder)
{
  trill_ioctl(circuit->fd, TRILL_VLANFWDER, forwarder);
}

void
trill_port_flush (struct isis_circuit *circuit, uint16_t vlan)
{
  trill_ioctl(circuit->fd, TRILL_PORTFLUSH, (void *)(unsigned long)vlan);
}

void
trill_nick_flush (struct isis_circuit *circuit, uint16_t vlan)
{
...
trill_nickdb_print (struct vty *vty, struct isis_area *area)
{
  ...
  for (dnode = dict_first(area->trill->nickdb);
       ...
       dnode = dict_next(area->trill->nickdb, dnode))
    {
      ...
      EACH_VLAN_R(tnode->info.vlans_forwarder, vlan, vlan_set)
        {
           if (!vlan_set)
             continue;
	   vlan_count++;
...
gather_bridge_ids(struct isis_area *area,
    struct trill_vlan_bridge_roots_subtlv_compat *vlantlv, int vlan)
{
  time_t now;
  int circnt;
  struct listnode *node;
 ...
 */
int
tlv_add_trill_nickname(trill_nickname_info_t *nick_info,
  struct stream *stream, struct  isis_area *area)
{
  size_t tlvstart;
  u_char tflags;
  struct trill_nickname_subtlv_compat tn;
  int rc;
  int vlan;
  int circnt;
  int nbytes;
  struct trill_vlan_bridge_roots_subtlv_compat *lastvlantlv, *nextvlantlv;

  tlvstart = stream_get_endp (stream);

  rc = add_tlv(ISIS_TLV_ROUTER_CAPABILITY, 0, NULL, stream);
  if (rc != ISIS_OK)
    return rc;
  ...
  nextvlantlv = (struct trill_vlan_bridge_roots_subtlv_compat *)
  ...
  while (vlan <= VLAN_MAX)
      ...
      for (vlan++; vlan <= VLAN_MAX; vlan++)
	{
      ...
      {
	tlvstart = stream_get_endp (stream);
	rc = add_tlv(ISIS_TLV_ROUTER_CAPABILITY, 0, NULL,
	    stream);
	if (rc != ISIS_OK)
	  break;
       ...
       "IS-IS TRILL nickname database\n")
{
  ...
  struct isis *isis = trill_first_instance();
       ...
       "<1-65534>\n")
{
  ...
  nickname = strtoul(argv[0]->arg, NULL, 10);
  if (nickname < RBRIDGE_NICKNAME_MIN + 1 || nickname > RBRIDGE_NICKNAME_MAX)
    {
      vty_out (vty, "TRILL nickname value out of range%s", VTY_NEWLINE);
      return CMD_WARNING;
    }
       ...
       "<1-127>\n")
{
  ...
  priority = strtoul(argv[0]->arg, NULL, 10);
  if (priority < MIN_RBRIDGE_PRIORITY || priority > MAX_RBRIDGE_PRIORITY)
    {
      vty_out (vty, "TRILL nickname priority value out of range%s",
          VTY_NEWLINE);
      return CMD_WARNING;
    }
       ...
       "instance name\n")
{
  ...
  (void) strlcpy(area->trill->name, argv[0]->arg, MAXLINKNAMELEN);
  return CMD_SUCCESS;
}
...
update_link(dladm_handle_t handle, datalink_id_t linkid, void *arg)
{
  ...
  if (status == DLADM_STATUS_OK)
    {
      ifp = if_get_by_name (linkname, area->isis->vrf_id, NULL);
      ifp->ifindex = linkid;
...
set_vlan(dladm_handle_t handle, datalink_id_t linkid, void *arg)
{
  ...
  ifp = if_lookup_by_index (vinfo.dv_linkid, area->isis->vrf_id);
  if (ifp != NULL)
  {
    circuit = ifp->info;
...
trill_internal_reload(struct isis_area *area)
{
   ...
   */
  vrf = vrf_lookup_by_id(area->isis->vrf_id);
  if (vrf != NULL)
...
trill_read_config (char **cfilep, int argc, char **argv)
{
  ...
  command_setup_early_logging("syslog", "warnings");
  ...
  area = isis_area_create(instname, VRF_DEFAULT_NAME);
  area->isis->trill_active = TRUE;
